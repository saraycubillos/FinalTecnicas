<%-- 
    Document   : prueba
    Created on : oct 14, 2021, 6:24:56 p.m.
    Author     : USER
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>JSP Page</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
    </head>
    <body>
        <div class="container"><h1>PROGRAMACIÓN PELICULAS</h1></div>
        
        <div class="container">
            <table class="table table-hover table-bordered table-dark">
                <thead>
                    <tr>
                        <th>Sala</th>
                        <th>Pelicula</th>
                        <th>Genero de pelicula</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>2</td>
                        <td>Abracadabra</td>
                        <td>Infantil/fantasía</td>
                        
                    </tr>
                    <tr>
                        <td>1</td>
                        <td>El extraño mundo de Jack</td>
                        <td>Infantil/fantasía</td>   
                    </tr>
                     <tr>
                        <td>1</td>
                        <td>La casa de los sustos</td>
                        <td>Terror/Fantasía</td>                        
                    </tr>
                     <tr>
                        <td>3</td>
                        <td>La purga</td>
                        <td>Terror/Acción</td>                        
                    </tr>
                    <tr>
                        <td>3</td>
                        <td>Hotel 3</td>
                        <td>Terror/Misterio</td>                        
                    </tr>
                    <tr>
                        <td>3</td>
                        <td>Déjame salir</td>
                        <td>Terror/Suspenso</td>                        
                    </tr>
                    <tr>
                        <td>2</td>
                        <td>Hotel transylvania</td>
                        <td>Infantil/Comedia</td>                        
                    </tr>
                    <tr>
                        <td>3</td>
                        <td>Donnie darko</td>
                        <td>Ciencia ficción/Fantasía</td>                        
                    </tr>
                </tbody>
            </table>
        </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
    </body>
</html>
